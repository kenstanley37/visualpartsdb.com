...
if(isset($_POST['myhist'])){
        $vpd = new VISUALDB;
        $vail = new VALIDATE;
        $$dateFrom = $vail->sanitizeString($_GET['dfrom']);
        $$dateTo = $vail->sanitizeString($_GET['dto']);
        $userID = $vail->sanitizeString($_GET['user']);
        $userFname = $vail->sanitizeString($_GET['user_fname']);
        $userLname = $vail->sanitizeString($_GET['user_lname']);
        ...
        try 
        {
            ...
            $conn = $db;
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); // Sets the error mode
            $stmt = $conn->prepare("SELECT * FROM sku_search 
            ...
            $i = 2;
                ...
                while ($row = $stmt->fetch(PDO::FETCH_NAMED)) {
                    $sheet->setCellValue('A'.$i, $row['sku_search_sku']);
                    $sheet->setCellValue('B'.$i, $row['sku_desc']);
                    $sheet->setCellValue('C'.$i, date($row['sku_search_date']));
                    $sheet->setCellValue('D'.$i, time($row['sku_search_date']));
                    $sheet->setCellValue('E'.$i, $row['user_fname']);
                    $sheet->setCellValue('F'.$i, $row['user_lname']);
                    $i++;
                }
